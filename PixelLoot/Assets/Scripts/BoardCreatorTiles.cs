...
public class BoardCreatorTiles : MonoBehaviour
{
        ...
        for (int i = 0; i < tiles.Length; i++)
        {
            for (int j = 0; j < tiles[i].Length; j++)
            {
                if (tiles[i][j] == TileType.Wall)
                {
                    ...
                    playArea.SetTile(new Vector3Int(i, j, 0), wallTiles[Random.Range(0, wallTiles.Length)]);
                }
                else
                {
                    //InstantiateFromArray(floorTiles, i, j);
                    playArea.SetTile(new Vector3Int(i, j, 0), floorTiles[Random.Range(0, floorTiles.Length)]);
                }
            }
        }
    }
    ...
    void InstantiateHorizontalOuterWall(float startingX, float endingX, float yCoord)
    {
        ...
        while (currentX <= endingX)
        {
            ...
            playArea.SetTile(new Vector3Int((int)currentX, (int)yCoord, 0), outerWallTile);
            currentX++;
        }
    }
    ...
    void InstantiateVerticalOuterWall(float xCoord, float startingY, float endingY)
    {
        ...
        {
            playArea.SetTile(new Vector3Int((int)xCoord, (int)currentY, 0), outerWallTile);
